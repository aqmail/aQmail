...
void mailfile(char *fn)
{
  ...
  substdio_fdbuf(&ss,read,0,buf,sizeof(buf));
  substdio_fdbuf(&ssout,write,fd,outbuf,sizeof(outbuf));
  if (substdio_put(&ssout,ufline.s,ufline.len)) goto WRITERRS;
  if (substdio_put(&ssout,rpline.s,rpline.len)) goto WRITERRS;
  if (substdio_put(&ssout,dtline.s,dtline.len)) goto WRITERRS;
  ...
  for (;;) {
    ...
    if (gfrom(messline.s,messline.len))
      if (substdio_bput(&ssout,">",1)) goto WRITERRS;
    if (substdio_bput(&ssout,messline.s,messline.len)) goto WRITERRS;
    if (!match) {
  ...
  if (substdio_bputs(&ssout,"\n")) goto WRITERRS;
  if (substdio_flush(&ssout)) goto WRITERRS;
