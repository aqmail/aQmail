...
void die_opent()
{
//  buffer_putsflush(buffer_2,"qmail-newu: fatal: unable to open var/users.tmp\n");
  buffer_putsflush(buffer_2,"qmail-newu: fatal: unable to open users/cdb.tmp\n");
  die_temp();
}
void die_writet()
{
//  buffer_putsflush(buffer_2,"qmail-newu: fatal: unable to write var/users.tmp\n");
  buffer_putsflush(buffer_2,"qmail-newu: fatal: unable to write users/cdb.tmp\n");
  die_temp();
}
void die_rename()
{
//  buffer_putsflush(buffer_2,"qmail-newu: fatal: unable to move var/users.tmp to users/cdb\n");
...
//  fd = open_read("var/users.txt");   // new location + name
  fd = open_read("users/assign");
  if (fd == -1) die_opena();

  buffer_init(&ssin,read,fd,inbuf,sizeof(inbuf));

//  fdtemp = open_trunc("var/users.tmp");
