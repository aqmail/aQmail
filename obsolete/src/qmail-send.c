...
void cleandied() 
{ 
 log1s("alert: oh no! lost qmail-clean connection! dying...\n");
 flagexitasap = 1; 
}
...
int getinfo(stralloc *sa,datetime_sec *dt,unsigned long id)
{
  ...
  substdio_fdbuf(&ss,read,fdnumber,buf,sizeof(buf));
  if (getln(&ss,&line,&match,'\0') == -1) { close(fdnumber); return 0; }
  close(fdnumber);
  if (!match) return 0;
...
// char todobufinfo[SUBSTDIO_OUTSIZE];
...
void todo_do(fd_set *rfds)
{
  ...
  fd = open_read(fn.s);
  if (fd == -1) { log3s("warning: unable to open ",fn.s,"\n"); return; }

  fnmake_mess(id);

 /* just for the statistics */
  if (stat(fn.s,&st) == -1) { log3s("warning: unable to stat ",fn.s,"\n"); goto fail; }

  for (c = 0;c < CHANNELS;++c) {
    fnmake_chanaddr(id,c);
    if (unlink(fn.s) == -1) 
      if (errno != error_noent) { log3s("warning: unable to unlink ",fn.s,"\n"); goto fail; }
  }

  fnmake_info(id);
  if (unlink(fn.s) == -1) 
    if (errno != error_noent) { log3s("warning: unable to unlink ",fn.s," for info\n"); goto fail; }

  fdnumber = open_excl(fn.s);
  if (fdnumber == -1) { log3s("warning: unable to create ",fn.s," for info\n"); goto fail; }

  strnum3[fmt_ulong(strnum3,id)] = 0;
  log3s("new msg ",strnum3,"\n");
  ...
  for (;;) {
    if (getln(&ss,&todoline,&match,'\0') == -1) {
      ...
      fnmake_todo(id);
      log3s("warning: trouble reading ",fn.s," for todo\n"); goto fail;
    }
    if (!match) break;
    ...
    switch(todoline.s[0]) {
      ...
      case 'F':
        if (substdio_putflush(&ssinfo,todoline.s,todoline.len) == -1) {
          fnmake_info(id);
          log3s("warning: trouble writing to ",fn.s," for todo\n"); goto fail;
        }
        log2s("info msg ",strnum3);
        strnum2[fmt_ulong(strnum2,(unsigned long) st.st_size)] = 0;
        log2s(": bytes ",strnum2);
        log1s(" from <"); logsafe(todoline.s + 1);
  ...
  fnmake_info(id);
  if (substdio_flush(&ssinfo) == -1) { log3s("warning: trouble writing to ",fn.s," for info\n"); goto fail; }
  if (fsync(fdnumber) == -1) { log3s("warning: trouble fsyncing ",fn.s," for info\n"); goto fail; }
  close(fdnumber); fdnumber = -1;

  for (c = 0;c < CHANNELS;++c)
    if (fdchan[c] != -1) {
      fnmake_chanaddr(id,c);
      if (substdio_flush(&sschan[c]) == -1) { log3s("warning: trouble writing to ",fn.s," in channel\n"); goto fail; }
      if (fsync(fdchan[c]) == -1) { log3s("warning: trouble fsyncing ",fn.s," in channel\n"); goto fail; }
