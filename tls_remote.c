...
// General rules
  ...
  if (constmap(&maptlsdestinations,"+*",2)) return 4;
  if (constmap(&maptlsdestinations,"-*",2)) return 3;

// Fall thru rules

  if (constmap(&maptlsdestinations,"*",1)) return 2; // CERT
...
/* Standard Cert (if any) */
  ...
  tlsdomaininfo = 0; 
  if (constmap(&mapdomaincerts,"*",1)) return 1;
